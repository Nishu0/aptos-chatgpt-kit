...
function isDeFiAsset(asset: Asset): boolean {
...
function getProtocolInfo(asset: Asset): ProtocolInfo {
  const symbol = asset.metadata?.symbol || "";
  const assetType = asset.asset_type || "";
...
async function handler(request: NextRequest) {
  ...
  try {
    ...
    // Fetch balance to get assets
    const balanceResponse = await fetch(
      `${request.nextUrl.origin}/api/wallet/balance?account=${walletAddress}`,
      {
        headers: {
    ...
    if (!balanceResponse.ok) {
      throw new Error(`Failed to fetch wallet balance: ${balanceResponse.status}`);
    }

    const balanceData = await balanceResponse.json();

    // Convert balance data to assets format
    const allAssets: Asset[] = [];
    
    // Add APT as an asset
    if (balanceData.apt) {
      allAssets.push({
        metadata: { symbol: "APT", name: "Aptos Coin" },
        asset_type: "0x1::aptos_coin::AptosCoin",
        value: balanceData.apt,
      });
    }
    ...
    if (balanceData.tokens && Array.isArray(balanceData.tokens)) {
      for (const token of balanceData.tokens) {
        allAssets.push({
          metadata: { symbol: token.symbol, name: token.symbol },
          asset_type: token.coinType,
    ...
    const defiAssets = allAssets
      .filter((asset) => isDeFiAsset(asset))
      .map((asset) => {
    ...
    // Group by protocol
    const protocolGroups = defiAssets.reduce((acc: Record<string, ProtocolGroup>, asset) => {
    ...
    const totalValue = defiAssets.reduce((sum, asset) => sum + (asset.value || 0), 0);
